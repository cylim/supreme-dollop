...
function Schedule() {
    ...
    () =>
      schedule
        ? [...schedule.options].sort(
            (a, b) =>
              b.availableCount - a.availableCount || a.startAt - b.startAt,
          )
        : [],
    [schedule],
  )
  ...
  const loadCalendar = async () => {
    ...
    try {
      const periods = await getBusyTimes({
        timeMin:
          Math.min(...schedule.options.map((option) => option.startAt)) -
          60_000,
        timeMax:
          Math.max(...schedule.options.map((option) => option.endAt)) + 60_000,
      })
      setBusy(periods)
    } catch (caught) {
      setMessage(
        caught instanceof Error ? caught.message : 'Calendar lookup failed.',
      )
    } finally {
  ...
  const saveVote = async () => {
    ...
    try {
      ...
      setMessage(
        caught instanceof Error ? caught.message : 'Could not save your vote.',
      )
    } finally {
      setSaving(false)
    }
  ...
  const confirmOption = async (
    optionId: (typeof schedule.options)[number]['id'],
  ) => {
    setSaving(true)
    setMessage(null)
    try {
  ...
  return (
    <main {...stylex.props(styles.main)}>
      <header {...stylex.props(styles.scheduleHeader)}>
        <div>
          ...
          {schedule.description && (
            <p {...stylex.props(styles.description)}>{schedule.description}</p>
          )}
          <p {...stylex.props(styles.meta)}>
            {schedule.durationMinutes} minutes · closes{' '}
            {formatFull(schedule.votingClosesAt)} · host timezone{' '}
            {schedule.timezone}
          </p>
        </div>
        <button
      ...
      {schedule.status === 'finalized' && (
        <section {...stylex.props(styles.finalCard)}>
          ...
          <h2 {...stylex.props(styles.finalTitle)}>
            {formatFull(
              schedule.options.find(
                (option) => option.id === schedule.selectedOptionId,
              )?.startAt ?? 0,
            )}
          </h2>
      ...
      <div {...stylex.props(styles.layout)}>
          ...
          <div {...stylex.props(styles.panelHeading)}>
            <div>
              <span {...stylex.props(styles.eyebrow)}>
                {schedule.status === 'awaiting_confirmation' && schedule.isHost
                  ? 'Choose the final time'
                  : 'Your availability'}
              </span>
          ...
          <div {...stylex.props(styles.optionList)}>
            {ranked.map((option, index) => {
              const conflicts = busy.some(
                (period) =>
                  option.startAt < period.endAt &&
                  option.endAt > period.startAt,
              )
              const isSelected = schedule.selectedOptionId === option.id
              const canSelectFinal =
                schedule.isHost && schedule.status === 'awaiting_confirmation'
              return (
                <article
                  key={option.id}
                  {...stylex.props(
                    styles.option,
                    isSelected && styles.optionFinal,
                ...
                >
                  ...
                  <div {...stylex.props(styles.optionCopy)}>
                    ...
                    <div {...stylex.props(styles.tags)}>
                      ...
                      {option.id === recommendedId && (
                        <span {...stylex.props(styles.recommended)}>
                          Best overlap
                        </span>
                      )}
                      {conflicts && (
                        <span {...stylex.props(styles.conflict)}>
                          Calendar conflict
                        </span>
                      )}
                    </div>
                  ...
                  {schedule.canVote && (
                    <button
                      ...
                      {...stylex.props(
                        styles.voteButton,
                        votes[option.id] && styles.voteYes,
                      )}
                      onClick={() =>
                        setVotes((current) => ({
                          ...current,
                  ...
                  {canSelectFinal && (
                    <button
                      ...
                      onClick={() => void confirmOption(option.id)}
                    >
                      Choose this time
                    </button>
                  )}
        ...
        <aside {...stylex.props(styles.side)}>
          <div {...stylex.props(styles.sideCard)}>
            ...
            <strong {...stylex.props(styles.bigNumber)}>
          ...
          <div {...stylex.props(styles.sideCard, styles.privacyCard)}>
            ...
            <p>
              Conflict checks use only busy intervals. We never request event
      ...
      <section {...stylex.props(styles.attached)}>
        <h2 {...stylex.props(styles.attachedTitle)}>
          Decisions for this event
        </h2>
          ...
          >
            ...
            <div {...stylex.props(styles.attachActions)}>
              <button
                ...
                {...stylex.props(
                  styles.voteButton,
                  attachMode === 'single' && styles.voteYes,
                )}
              ...
              <button
                ...
                {...stylex.props(
                  styles.voteButton,
                  attachMode === 'multi' && styles.voteYes,
                )}
...
const styles = stylex.create({
  ...
  optionFinal: {
    borderColor: '#25704a',
    backgroundColor: '#dff2e4',
    boxShadow: 'inset 0 0 0 1px #25704a',
  },
  rank: {
    width: 30,
    height: 30,
