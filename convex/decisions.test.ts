/// <reference types="vite/client" />

import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { convexTest } from 'convex-test'
import schema from './schema'
import { api } from './_generated/api'
import type { TestConvex } from 'convex-test'
import type { Id } from './_generated/dataModel'

const modules = import.meta.glob('./**/*.ts')
const now = Date.UTC(2026, 8, 8, 12)

type TestBackend = TestConvex<typeof schema>

async function insertUser(
  t: TestBackend,
  email: string,
  extra: { name?: string; picture?: string } = {},
): Promise<Id<'users'>> {
  return await t.run(async (ctx) =>
    ctx.db.insert('users', {
      providerAccountId: `google:${email}`,
      email,
      ...extra,
    }),
  )
}

function asUser(t: TestBackend, id: Id<'users'>, email: string) {
  return t.withIdentity({
    subject: id,
    tokenIdentifier: `https://test.invalid|${id}`,
    email,
  })
}

function standaloneInput(
  visibility: 'public' | 'invited',
  inviteEmails: Array<string> = [],
) {
  return {
    title: 'Beach or park?',
    visibility,
    selectMode: 'single' as const,
    options: ['Beach', 'Park'],
    inviteEmails,
  }
}

describe('decisions', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    vi.setSystemTime(now)
  })

  afterEach(() => {
    vi.useRealTimers()
  })

  it('rejects anonymous callers across the public decision API', async () => {
    const t = convexTest(schema, modules)
    const hostId = await insertUser(t, 'host@example.test')
    const host = asUser(t, hostId, 'host@example.test')
    const decision = await host.mutation(
      api.decisions.create,
      standaloneInput('public'),
    )
    const view = await host.query(api.decisions.getBySlug, {
      slug: decision.slug,
    })
    const schedule = await host.mutation(api.schedules.create, {
      title: 'Schedule',
      visibility: 'public',
      timezone: 'Asia/Kuala_Lumpur',
      durationMinutes: 60,
      votingClosesAt: now + 60_001,
      options: [
        {
          startAt: now + 120_000,
          endAt: now + 180_000,
          source: 'exact',
        },
        {
          startAt: now + 240_000,
          endAt: now + 300_000,
          source: 'exact',
        },
      ],
      inviteEmails: [],
    })
    const optionId = view!.options[0].id
    const calls = [
      () => t.query(api.decisions.listMine, {}),
      () => t.query(api.decisions.getBySlug, { slug: decision.slug }),
      () => t.query(api.decisions.listAttached, { scheduleId: schedule.id }),
      () =>
        t.mutation(api.decisions.createAttached, {
          scheduleId: schedule.id,
          title: 'Attached',
          selectMode: 'single',
          options: ['One', 'Two'],
        }),
      () =>
        t.mutation(api.decisions.submitBallot, {
          decisionId: decision.id,
          optionIds: [optionId],
        }),
      () =>
        t.mutation(api.decisions.addOption, {
          decisionId: decision.id,
          label: 'Three',
        }),
      () =>
        t.mutation(api.decisions.renameOption, {
          decisionId: decision.id,
          optionId,
          label: 'Renamed',
        }),
      () =>
        t.mutation(api.decisions.removeOption, {
          decisionId: decision.id,
          optionId,
        }),
      () =>
        t.mutation(api.decisions.update, {
          decisionId: decision.id,
          title: 'Updated',
        }),
      () => t.mutation(api.decisions.close, { decisionId: decision.id }),
    ]
    for (const call of calls) await expect(call()).rejects.toThrow(/signed in/i)
  })

  it('lets any signed-in user ballot a public decision and shows live faces', async () => {
    const t = convexTest(schema, modules)
    const hostId = await insertUser(t, 'host@example.test', {
      name: 'Alex Host',
    })
    const participantId = await insertUser(t, 'maya@example.test', {
      name: 'Maya Guest',
      picture: 'https://example.test/maya.png',
    })
    const host = asUser(t, hostId, 'host@example.test')
    const participant = asUser(t, participantId, 'maya@example.test')

    await expect(
      t.mutation(api.decisions.create, standaloneInput('public')),
    ).rejects.toThrow(/signed in/i)

    const created = await host.mutation(
      api.decisions.create,
      standaloneInput('public'),
    )
    const view = await participant.query(api.decisions.getBySlug, {
      slug: created.slug,
    })
    expect(view).toMatchObject({
      title: 'Beach or park?',
      selectMode: 'single',
      status: 'open',
      canSubmitBallot: true,
      participantCount: 0,
    })
    const beach = view!.options.find((option) => option.label === 'Beach')!
    await participant.mutation(api.decisions.submitBallot, {
      decisionId: created.id,
      optionIds: [beach.id],
    })

    const after = await host.query(api.decisions.getBySlug, {
      slug: created.slug,
    })
    const beachAfter = after!.options.find(
      (option) => option.label === 'Beach',
    )!
    expect(beachAfter.selectionCount).toBe(1)
    expect(beachAfter.faces).toEqual([
      { picture: 'https://example.test/maya.png', initial: 'M' },
    ])
    expect(after!.participantCount).toBe(1)
    const mine = await host.query(api.decisions.listMine, {})
    expect(mine).toHaveLength(1)
    expect(mine[0].id).toBe(created.id)
  })

  it('rejects outsiders on invited decisions and queues invitation mail', async () => {
    const t = convexTest(schema, modules)
    const hostId = await insertUser(t, 'host@example.test')
    const inviteeId = await insertUser(t, 'guest@example.test')
    const outsiderId = await insertUser(t, 'outsider@example.test')
    const host = asUser(t, hostId, 'host@example.test')
    const invitee = asUser(t, inviteeId, 'guest@example.test')
    const outsider = asUser(t, outsiderId, 'outsider@example.test')

    const created = await host.mutation(
      api.decisions.create,
      standaloneInput('invited', ['Guest@Example.Test']),
    )
    await expect(
      outsider.query(api.decisions.getBySlug, { slug: created.slug }),
    ).rejects.toThrow(/invited/i)
    const inviteeView = await invitee.query(api.decisions.getBySlug, {
      slug: created.slug,
    })
    expect(inviteeView).not.toBeNull()
    await expect(
      outsider.mutation(api.decisions.submitBallot, {
        decisionId: created.id,
        optionIds: [inviteeView!.options[0].id],
      }),
    ).rejects.toThrow(/invitation/i)
    expect(await outsider.query(api.decisions.listMine, {})).toEqual([])
    expect(
      await host.query(api.decisions.getBySlug, { slug: created.slug }),
    ).toMatchObject({
      invitations: [{ email: 'guest@example.test' }],
    })
    const runs = await t.run(async (ctx) =>
      ctx.db
        .query('notificationRuns')
        .withIndex('by_decision_and_kind', (q) =>
          q.eq('decisionId', created.id).eq('kind', 'decision_invitation'),
        )
        .take(10),
    )
    expect(runs).toHaveLength(1)
  })

  it('supports multi select, retract, option edits, and final close', async () => {
    const t = convexTest(schema, modules)
    const hostId = await insertUser(t, 'host@example.test', { name: 'Alex' })
    const participantId = await insertUser(t, 'maya@example.test', {
      name: 'Maya',
    })
    const host = asUser(t, hostId, 'host@example.test')
    const participant = asUser(t, participantId, 'maya@example.test')

    const created = await host.mutation(api.decisions.create, {
      ...standaloneInput('public'),
      selectMode: 'multi',
      options: ['Thai', 'Pizza', 'Sushi'],
    })
    const view = await participant.query(api.decisions.getBySlug, {
      slug: created.slug,
    })
    const byLabel = Object.fromEntries(
      view!.options.map((option) => [option.label, option.id]),
    )
    await participant.mutation(api.decisions.submitBallot, {
      decisionId: created.id,
      optionIds: [byLabel.Thai, byLabel.Pizza],
    })
    await expect(
      host.mutation(api.decisions.removeOption, {
        decisionId: created.id,
        optionId: byLabel.Thai,
      }),
    ).rejects.toThrow(/has a selection/i)

    await host.mutation(api.decisions.addOption, {
      decisionId: created.id,
      label: 'Ramen',
    })
    await host.mutation(api.decisions.renameOption, {
      decisionId: created.id,
      optionId: byLabel.Sushi,
      label: 'Sushi roll',
    })

    await participant.mutation(api.decisions.submitBallot, {
      decisionId: created.id,
      optionIds: [byLabel.Pizza],
    })
    await host.mutation(api.decisions.removeOption, {
      decisionId: created.id,
      optionId: byLabel.Thai,
    })

    await participant.mutation(api.decisions.submitBallot, {
      decisionId: created.id,
      optionIds: [],
    })
    const retracted = await host.query(api.decisions.getBySlug, {
      slug: created.slug,
    })
    expect(retracted!.participantCount).toBe(0)
    expect(
      retracted!.options.every((option) => option.selectionCount === 0),
    ).toBe(true)

    await host.mutation(api.decisions.close, { decisionId: created.id })
    await expect(
      participant.mutation(api.decisions.submitBallot, {
        decisionId: created.id,
        optionIds: [byLabel.Pizza],
      }),
    ).rejects.toThrow(/closed/i)
    const closed = await participant.query(api.decisions.getBySlug, {
      slug: created.slug,
    })
    expect(closed).toMatchObject({
      status: 'closed',
      canSubmitBallot: false,
    })
  })

  it('materializes deadline closure through the scheduled callback', async () => {
    const t = convexTest(schema, modules)
    const hostId = await insertUser(t, 'host@example.test')
    const host = asUser(t, hostId, 'host@example.test')
    const closesAt = now + 120_000
    const created = await host.mutation(api.decisions.create, {
      ...standaloneInput('public'),
      closesAt,
    })

    vi.setSystemTime(closesAt + 1)
    await t.finishAllScheduledFunctions(vi.runOnlyPendingTimers)
    const view = await host.query(api.decisions.getBySlug, {
      slug: created.slug,
    })
    expect(view).toMatchObject({
      status: 'closed',
      canSubmitBallot: false,
    })
    await expect(
      host.mutation(api.decisions.addOption, {
        decisionId: created.id,
        label: 'Museum',
      }),
    ).rejects.toThrow(/closed/i)
    await expect(
      host.mutation(api.decisions.update, {
        decisionId: created.id,
        title: 'Too late',
      }),
    ).rejects.toThrow(/closed/i)
    await expect(
      host.mutation(api.decisions.renameOption, {
        decisionId: created.id,
        optionId: view!.options[0].id,
        label: 'Too late',
      }),
    ).rejects.toThrow(/closed/i)
    await expect(
      host.mutation(api.decisions.removeOption, {
        decisionId: created.id,
        optionId: view!.options[0].id,
      }),
    ).rejects.toThrow(/closed/i)
    await expect(
      host.mutation(api.decisions.submitBallot, {
        decisionId: created.id,
        optionIds: [view!.options[0].id],
      }),
    ).rejects.toThrow(/closed/i)
  })

  it('attaches decisions to a schedule and keeps them open after the final time', async () => {
    const t = convexTest(schema, modules)
    const hostId = await insertUser(t, 'host@example.test')
    const inviteeId = await insertUser(t, 'guest@example.test')
    const outsiderId = await insertUser(t, 'outsider@example.test')
    const host = asUser(t, hostId, 'host@example.test')
    const invitee = asUser(t, inviteeId, 'guest@example.test')
    const outsider = asUser(t, outsiderId, 'outsider@example.test')

    const schedule = await host.mutation(api.schedules.create, {
      title: 'Saturday dinner',
      visibility: 'invited',
      timezone: 'Asia/Kuala_Lumpur',
      durationMinutes: 60,
      votingClosesAt: now + 86_400_000,
      options: [
        {
          startAt: now + 172_800_000,
          endAt: now + 176_400_000,
          source: 'exact',
        },
        {
          startAt: now + 259_200_000,
          endAt: now + 262_800_000,
          source: 'exact',
        },
      ],
      inviteEmails: ['guest@example.test'],
    })

    await expect(
      invitee.mutation(api.decisions.createAttached, {
        scheduleId: schedule.id,
        title: 'What do we eat?',
        selectMode: 'single',
        options: ['Thai', 'Pizza'],
      }),
    ).rejects.toThrow(/host/i)
    await expect(
      outsider.mutation(api.decisions.createAttached, {
        scheduleId: schedule.id,
        title: 'Outsider decision',
        selectMode: 'single',
        options: ['One', 'Two'],
      }),
    ).rejects.toThrow(/host/i)

    const attached = await host.mutation(api.decisions.createAttached, {
      scheduleId: schedule.id,
      title: 'What do we eat?',
      selectMode: 'single',
      options: ['Thai', 'Pizza'],
    })
    await expect(
      outsider.query(api.decisions.getBySlug, { slug: attached.slug }),
    ).rejects.toThrow(/invited/i)
    const food = await invitee.query(api.decisions.getBySlug, {
      slug: attached.slug,
    })
    expect(food).toMatchObject({
      title: 'What do we eat?',
      scheduleId: schedule.id,
    })
    const thai = food!.options.find((option) => option.label === 'Thai')!
    await invitee.mutation(api.decisions.submitBallot, {
      decisionId: attached.id,
      optionIds: [thai.id],
    })

    const listed = await invitee.query(api.decisions.listAttached, {
      scheduleId: schedule.id,
    })
    expect(listed).toHaveLength(1)
    expect(listed[0].options.every((option) => option.faces.length === 0)).toBe(
      true,
    )
    await expect(
      outsider.query(api.decisions.listAttached, { scheduleId: schedule.id }),
    ).rejects.toThrow(/invited/i)

    const scheduleView = await host.query(api.schedules.getBySlug, {
      slug: schedule.slug,
      now,
    })
    await invitee.mutation(api.schedules.submitVote, {
      scheduleId: schedule.id,
      responses: scheduleView!.options.map((option) => ({
        optionId: option.id,
        available: true,
      })),
    })
    const awaiting = await host.query(api.schedules.getBySlug, {
      slug: schedule.slug,
      now,
    })
    await host.mutation(api.schedules.chooseFinal, {
      scheduleId: schedule.id,
      optionId: awaiting!.options[0].id,
    })
    const stillOpen = await invitee.query(api.decisions.getBySlug, {
      slug: attached.slug,
    })
    expect(stillOpen).toMatchObject({
      status: 'open',
      canSubmitBallot: true,
    })

    const mine = await host.query(api.decisions.listMine, {})
    expect(mine).toHaveLength(0)

    await t.run(async (ctx) => ctx.db.delete('schedules', schedule.id))
    await expect(
      invitee.query(api.decisions.getBySlug, { slug: attached.slug }),
    ).rejects.toThrow(/schedule not found/i)
  })

  it('enforces host ownership, option ownership, and single-select invariants', async () => {
    const t = convexTest(schema, modules)
    const hostId = await insertUser(t, 'host@example.test')
    const participantId = await insertUser(t, 'guest@example.test')
    const outsiderId = await insertUser(t, 'outsider@example.test')
    const host = asUser(t, hostId, 'host@example.test')
    const participant = asUser(t, participantId, 'guest@example.test')
    const outsider = asUser(t, outsiderId, 'outsider@example.test')
    const first = await host.mutation(
      api.decisions.create,
      standaloneInput('public'),
    )
    const second = await host.mutation(api.decisions.create, {
      ...standaloneInput('public'),
      title: 'Second decision',
    })
    const firstView = await participant.query(api.decisions.getBySlug, {
      slug: first.slug,
    })
    const secondView = await participant.query(api.decisions.getBySlug, {
      slug: second.slug,
    })
    const optionIds = firstView!.options.map((option) => option.id)

    await participant.mutation(api.decisions.submitBallot, {
      decisionId: first.id,
      optionIds: [optionIds[0]],
    })

    await expect(
      t.mutation(api.decisions.update, {
        decisionId: first.id,
        title: 'Changed',
      }),
    ).rejects.toThrow(/signed in/i)
    for (const caller of [participant, outsider]) {
      await expect(
        caller.mutation(api.decisions.update, {
          decisionId: first.id,
          title: 'Changed',
        }),
      ).rejects.toThrow(/host/i)
      await expect(
        caller.mutation(api.decisions.close, { decisionId: first.id }),
      ).rejects.toThrow(/host/i)
      await expect(
        caller.mutation(api.decisions.addOption, {
          decisionId: first.id,
          label: 'Museum',
        }),
      ).rejects.toThrow(/host/i)
      await expect(
        caller.mutation(api.decisions.renameOption, {
          decisionId: first.id,
          optionId: optionIds[1],
          label: 'Renamed',
        }),
      ).rejects.toThrow(/host/i)
      await expect(
        caller.mutation(api.decisions.removeOption, {
          decisionId: first.id,
          optionId: optionIds[1],
        }),
      ).rejects.toThrow(/host/i)
    }
    await expect(
      participant.mutation(api.decisions.submitBallot, {
        decisionId: first.id,
        optionIds: [optionIds[0], optionIds[0]],
      }),
    ).rejects.toThrow(/once/i)
    await expect(
      participant.mutation(api.decisions.submitBallot, {
        decisionId: first.id,
        optionIds,
      }),
    ).rejects.toThrow(/pick one/i)
    await expect(
      participant.mutation(api.decisions.submitBallot, {
        decisionId: first.id,
        optionIds: [secondView!.options[0].id],
      }),
    ).rejects.toThrow(/does not belong/i)
    await expect(
      host.mutation(api.decisions.renameOption, {
        decisionId: first.id,
        optionId: secondView!.options[0].id,
        label: 'Wrong decision',
      }),
    ).rejects.toThrow(/does not belong/i)

    await host.mutation(api.decisions.update, {
      decisionId: first.id,
      title: 'Updated title',
      description: 'Updated description',
      closesAt: now + 60_001,
    })
    const updated = await host.query(api.decisions.getBySlug, {
      slug: first.slug,
    })
    expect(updated).toMatchObject({
      title: 'Updated title',
      description: 'Updated description',
      closesAt: now + 60_001,
    })
    await host.mutation(api.decisions.update, {
      decisionId: first.id,
      description: '   ',
      closesAt: null,
    })
    expect(
      await host.query(api.decisions.getBySlug, { slug: first.slug }),
    ).toMatchObject({ closesAt: null })

    await host.mutation(api.decisions.close, { decisionId: first.id })
    await host.mutation(api.decisions.close, { decisionId: first.id })
    await expect(
      host.mutation(api.decisions.update, {
        decisionId: first.id,
        title: 'Too late',
      }),
    ).rejects.toThrow(/closed/i)
  })

  it('keeps exact ballot counts beyond the bounded face preview', async () => {
    const t = convexTest(schema, modules)
    const hostId = await insertUser(t, 'host@example.test')
    const host = asUser(t, hostId, 'host@example.test')
    const created = await host.mutation(
      api.decisions.create,
      standaloneInput('public'),
    )
    const initial = await host.query(api.decisions.getBySlug, {
      slug: created.slug,
    })
    const optionId = initial!.options[0].id

    for (let index = 0; index < 251; index += 1) {
      const email = `participant-${index}@example.test`
      const participantId = await insertUser(t, email)
      await asUser(t, participantId, email).mutation(
        api.decisions.submitBallot,
        { decisionId: created.id, optionIds: [optionId] },
      )
    }

    const view = await host.query(api.decisions.getBySlug, {
      slug: created.slug,
    })
    const selected = view!.options.find((option) => option.id === optionId)!
    expect(view!.participantCount).toBe(251)
    expect(selected.selectionCount).toBe(251)
    expect(selected.faces).toHaveLength(12)
    const storedBallots = await t.run(async (ctx) =>
      ctx.db
        .query('decisionParticipants')
        .withIndex('by_decision', (query) => query.eq('decisionId', created.id))
        .take(300),
    )
    expect(storedBallots).toHaveLength(251)

    const legacy = await host.mutation(
      api.decisions.create,
      standaloneInput('public'),
    )
    const legacyView = await host.query(api.decisions.getBySlug, {
      slug: legacy.slug,
    })
    const legacyParticipantId = await insertUser(t, 'legacy@example.test')
    await asUser(t, legacyParticipantId, 'legacy@example.test').mutation(
      api.decisions.submitBallot,
      { decisionId: legacy.id, optionIds: [legacyView!.options[0].id] },
    )
    await t.run(async (ctx) =>
      ctx.db.patch('decisions', legacy.id, { participantCount: undefined }),
    )
    expect(
      await host.query(api.decisions.getBySlug, { slug: legacy.slug }),
    ).toMatchObject({ participantCount: 1 })
  })
})
